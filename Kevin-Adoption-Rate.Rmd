...
---
title: "Animal Adoption Rate"
author: "Ashley Kim, Gwen Powers, Kevin Cooper"
output:
  html_document: 
    toc: TRUE
    theme: simplex
...
<img src="adopt_me.jpg" alt="Adopt Me Picture" width="450" height="256">
...
abc <- names(select_if(shelter_data2, is.numeric))

shelter_data2[abc] <- lapply(shelter_data2[abc], normalize)
```

```{r echo=FALSE}
# one hot encoding
ab_c <- names(select_if(shelter_data2, is.factor))
shelter_data2 <- one_hot(as.data.table(shelter_data2),cols=ab_c,sparsifyNAs = TRUE,naCols = FALSE,dropCols = TRUE,dropUnusedLevels = TRUE)

shelter_data2 <- shelter_data2[,-29]
```

```{r echo=FALSE}
# Train Tune Test split at 60/20/20
part_index_1 <- createDataPartition(shelter_data2$Outcome.Type_Adopted,
                                           times=1,
                                           p = 0.60,
                                           groups=1,
                                           list=FALSE)
...
tune_and_test_index <- createDataPartition(tune_and_test$Outcome.Type_Adopted,
                                           p = .5,
                                           list = FALSE,
                                           times = 1)
...
predictandCM<- function(model,data,modeltype,ref)
{
  ...
  confusionMatrix(pred, reference=ref, positive = 'Adopted')
}
```
...
predictandCM<- function(model,data,modeltype,ref)
{
  #model using, data going into the model, and output type for predict function
  pred <-predict(model,data,type=modeltype)
  confusionMatrix(pred, reference=ref, positive = 'Rescued')
}
```
...
<img src="puppy.jpg" alt="Puppy Picture" width="200" height="150">
...
<img src="old_dog.jpg" alt="Old Dog Picture" width="474" height="237">

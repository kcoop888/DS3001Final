...
```{r}
date_data <-  read_csv("/Users/ashleykim/Documents/Y3 2021-2022/DS3001Final/animal-shelter-intakes-and-outcomes.csv")

date_data <- date_data[,-c(1,2,5,7,12,13,15,18,19,20)] 

date_data<- na.omit(date_data)

date_data <-  date_data[,c(5,8)]
```

```{r}
date_data[,c(1,2)] <- lapply(date_data[,c(1,2)], as.Date)

dates <- data.frame(date_data$`Outcome Date` - date_data$`Intake Date`)
#table(dates$date_data..Outcome.Date....date_data..Intake.Date.)

dates <- data.frame(as.numeric(difftime(date_data$`Outcome Date`, date_data$`Intake Date`, units = "days")))
colnames(dates) <- c("days_in_shelter")
```
...
shelter_data <- shelter_data[,-c(1,2,5,7,9,12,13,14,15,18,19,20)] # dropping animal name, secondary color, reason for intake, 
...
## Future work
- maybe more quantitative variables?
